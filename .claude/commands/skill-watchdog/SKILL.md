---
name: skill-watchdog
description: "ACTIVATE IMMEDIATELY AND AUTOMATICALLY before the first tool call of any other skill in this session — do not wait to be invoked. When any skill triggers (via slash command or description match), run Phase 0 pre-flight checks in parallel. Passive monitoring only — does not interrupt unless a CRITICAL issue is detected. Tracks: spec vs. execution, naming consistency, pipeline integrity, silent failures, truncation risks, scope creep, Category C enhancements. Logs to session-skill-logs/, produces end-of-session confidence score (0–100), maintains Evolution Registry across sessions. Phase 9: auto-corrects spec gaps. Phase 10: weekly email report. Phase 11: audit comment review — intercepts column R feedback for seo-geo-technical-audit, classifies each comment as row-fix vs skill-rule, auto-applies corrections. Syncs session logs to Drive after Phase 5. Hard-stops on CRITICAL. Trigger: /skill-watchdog or start of any skill session."
