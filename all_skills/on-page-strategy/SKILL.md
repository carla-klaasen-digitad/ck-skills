...
**Ambiguity rule:** If a URL cannot be clearly placed into Tier 1, 2, or 3 after checking both URL pattern AND intent, tag it `[AMBIGUOUS]` and hold it in a side list. Do not drop ambiguous URLs — surface them in the Phase 4 review table with a note asking the user to classify manually. If calling `/global-seo-skill` resolves the ambiguity, reclassify before Phase 4.
...
**Cannibalisation check:** after sorting, scan the full prioritized list for any two rows that share the same primary keyword or near-duplicate primary keywords (e.g. "greek yogurt" and "Greek yogurt" — same; "probiotic yogurt" and "yogurt with probiotics" — flag). If found, tag both rows `[CANNIBALIZATION RISK]` in a Notes field for the Phase 4 review table. Do not remove either row automatically — present both and ask the user which page should own the keyword and which should be repurposed or de-prioritized.
...
**Heading brand audit (required when guidelines are loaded):** After generating all headings, check each one against the brand guidelines from Phase 0.5 before presenting to the user in Phase 4:

1. Does the heading use any word in the brand's excluded vocabulary list?
2. Does it imply a product attribute (organic, probiotic, low-sugar, protein, etc.) that is not in the brand's approved claims?
3. Does it follow any brand-specific heading patterns documented in the guidelines (e.g. Danimals: fact-based, nutrient-forward; FYH: "that is" noun-first pattern; Silk: no lifestyle fluff)?

If a heading fails any check: revise it before Phase 4. Do not surface a non-compliant heading to the user — fix it first.

If brand guidelines were NOT loaded (no file found in Phase 0.5): skip this audit and apply generic SEO heading rules only.
...
Then ask: **"Does this plan look right? Before confirming, review: (1) headings — do they match the brand voice? (2) any `[CANNIBALIZATION RISK]` or `[AMBIGUOUS]` rows flagged above — which page wins the keyword? (3) exclusions — correct pages cut? Type 'confirm' to write to the sheet, or tell me what to adjust."**
